...
**버전 0.21** — 이제 맥북·맥에서도 주식을 사고팔 수 있습니다.
...
첫 화면이 종합(`/`, `/trade`)입니다. 상단 지수·환율, 호가, 주문, 잔고, 관심종목, 주문내역/당일 체결/거래내역 탭. **종합2**(`/desk2`)는 관심종목·큰 차트·호가·주문을 한 줄로 둔 다른 배치입니다. 로그, 설정.
...
`VERSION`이 제품 버전입니다. 지금은 **0.21**이고 다음은 **0.22**입니다.
